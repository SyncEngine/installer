...
function headerHtml($title)
{
    ...
    <button class="btn btn-link position-fixed top-0 end-0 p-3 text-decoration-none" onclick="toggleTheme()" title="Toggle dark/light mode" style="z-index: 1000;">
...
function footerHtml()
{
    ...
    <script>
        ...
        function updateIcons(theme) {
            document.querySelector('.theme-icon-light').style.display = theme === 'dark' ? '' : 'none';
            document.querySelector('.theme-icon-dark').style.display = theme === 'light' ? '' : 'none';
        }
        (function() {
            var stored = localStorage.getItem('theme');
            var theme;
